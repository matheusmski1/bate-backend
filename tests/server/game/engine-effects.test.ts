...
function stateWithEffect(type: 'peek-own' | 'peek-other' | 'swap'): GameState {
  const p1: Player = { id: 'p1', socketId: null, name: 'A', hand: [card('5'), card('K'), card('3'), card('9')], score: 0, connected: true, disconnectedAt: null, revealedToSelf: [] }
  const p2: Player = { id: 'p2', socketId: null, name: 'B', hand: [card('A'), card('7'), card('Q'), card('2')], score: 0, connected: true, disconnectedAt: null, revealedToSelf: [] }
  return {
